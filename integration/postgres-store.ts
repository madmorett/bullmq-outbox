    ...
    async markFailed(failure) {
      await pool.query(
        ...
        [
          ...
          failure.attempts,
          failure.error,
          // An expired entry must stop coming back from loadPending. Keeping
          // the row (rather than deleting it) is what lets someone look at
          // what was lost on Monday morning.
          failure.expired ? 'expired' : 'pending',
        ],
      );
    },
  };
